...
import { sand, sandDark, blue, blueDark } from '@radix-ui/colors';

const transformRadixToChakraFormat = (scale: any) => {
  const output = Object.values(scale).reduce((accumulator: any, currentValue, index) => {
    if (index === 0) {
...
const radixNeutral: any = transformRadixToChakraFormat(sand);
const radixNeutralDark: any = transformRadixToChakraFormat(sandDark);
const radixPrimary: any = transformRadixToChakraFormat(blue);
const radixPrimaryD: any = transformRadixToChakraFormat(blueDark);
